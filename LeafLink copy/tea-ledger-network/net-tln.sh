...
function printHelp() {
  ...
  echo "    -c <channel name> - channel name to use (defaults to \"tlnchannel\")"
  ...
  echo
  echo " Examples:"
  echo "  net-tln.sh up createChannel"
  echo "  net-tln.sh down"
  echo "  net-tln.sh deploySmartContract"
}
...
function createConsortium() {
  ...
  set -x
  configtxgen -profile TeaLedgerOrdererGenesis -channelID system-channel -outputBlock ./system-genesis-block/genesis.block
  res=$?
  set +x
  if [ $res -ne 0 ]; then
...
CLI_DELAY=3
# channel name defaults to "tlnchannel"
CHANNEL_NAME="tlnchannel"
# use this as the default docker-compose yaml definition
COMPOSE_FILE_BASE=docker/docker-compose-tln-net.yaml
...
if [ "$MODE" == "up" ]; then
  ...
  echo "Starting docker log monitoring on network '${COMPOSE_PROJECT_NAME}_tln'"
  echo
elif [ "$MODE" == "monitor-down" ]; then
  echo "Stopping docker log monitoring on network '${COMPOSE_PROJECT_NAME}_tln'"
  echo
else
  printHelp
  exit 1
