...
app.post('/makeEquipment', async (req, res, next) => {
  ...
  try {
    if(!userName || userName.lenth<1) {
      return res.status(500).json("User is missing");
    } else if (!farmer || !equipmentName || !ownerName || !equipmentNumber) {
      return res.status(500).json("Missing requied fields: farmer, equipmentName, ownerName, equipmentNumber");
    } else {
      const result = await await equipmentSvcInstance.makeEquipment(userName, farmer, equipmentNumber, equipmentName, ownerName);
