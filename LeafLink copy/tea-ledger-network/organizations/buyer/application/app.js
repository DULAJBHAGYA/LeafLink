...
const TeacyService = require('./services/buyerService.js');
...
const buyerServiceSvcInstance = new TeacyService();
...
app.post('/buyerReceived', async (req, res, next) => {
  ...
  try {
    if(!userName || userName.lenth<1) {
      ...
      const result = await await buyerServiceSvcInstance.buyerReceived(userName, equipmentNumber, ownerName);
