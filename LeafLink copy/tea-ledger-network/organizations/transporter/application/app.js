...
const WholesalerService = require('./services/transporterService.js');
...
const transporterSvcInstance = new WholesalerService();
...
app.post('/transporterDistribute', async (req, res, next) => {
  ...
  try {
    if(!userName || userName.lenth<1) {
      ...
      const result = await await transporterSvcInstance.transporterDistribute(userName, equipmentNumber, ownerName);
