...
**  Author: Brian Wu
** JS for transporter web appication
**/
$(document).ready(function(){
    ...
    var urlBase = "http://54.161.38.96:30001";
     var tabs =["addToWallet", "transporterDistribute", "query", "queryHistory"];
     $("#queryResult").hide();
     $("#addToWalletLink").click(function(){
       showTab("addToWallet");
     });
   $("#transporterDistributeLink").click(function(){
     showTab("transporterDistribute");
   });
 ...
 $("#transporterDistribute").click(function(){
   var transporterDistributeUrl = urlBase+"/transporterDistribute";
   var formData = {
   ...
   $.ajax({
     type: 'POST',
     url: transporterDistributeUrl,
     data: formData,
     success: function(data, status, jqXHR){
       if(status==='success'){
         alert("successfully record transporter Distribute  in blockchain");
      }
       showTab("query");
     },
     error: function(xhr, textStatus, error){
 ...
 $.ajax({
   ...
   success: function(data, status, jqXHR){
     if(!data || !data.Record || !data.Record.equipmentNumber) {
       ...
       $("#farmerOutPut").text(record.farmer);
