...
CC_SRC_PATH="organizations/buyer/contract/"
...
echo
echo "Deploy smart contract Tea Ledger Network (PLN) fabric blockchain"
echo
...
packageChaincode() {
  ...
  set -x
  docker exec cli peer lifecycle chaincode package ${CHINCODE_NAME}.tar.gz --path ${CC_SRC_PATH} --lang ${CC_RUNTIME_LANGUAGE} --label ${CHINCODE_NAME}_${VERSION} >&log.txt
  res=$?
  set +x
  cat log.txt
...
installChaincode() {
  ...
  echo "Waiting for 15 seconds before installing chaincode on peer0.org${ORG}..."
  sleep 15
  
  # Set peer port based on organization
  case $ORG in
    1) PEER_PORT=7051 ;;
    2) PEER_PORT=9051 ;;
    3) PEER_PORT=11051 ;;
  ...
  set -x
  docker exec -e CORE_PEER_LOCALMSPID=Org${ORG}MSP -e CORE_PEER_ADDRESS=peer0.org${ORG}.example.com:${PEER_PORT} -e CORE_PEER_MSPCONFIGPATH=/opt/gopath/src/github.com/hyperledger/fabric/peer/organizations/peerOrganizations/org${ORG}.example.com/users/Admin@org${ORG}.example.com/msp -e CORE_PEER_TLS_ROOTCERT_FILE=/opt/gopath/src/github.com/hyperledger/fabric/peer/organizations/peerOrganizations/org${ORG}.example.com/peers/peer0.org${ORG}.example.com/tls/ca.crt cli peer lifecycle chaincode queryinstalled >&log.txt
  res=$?
  set +x
  cat log.txt
  # Check if the new version is already installed
  if [ -z "$(cat log.txt | grep -o "Package ID: .*${CHINCODE_NAME}_${VERSION}")" ]; then
    echo "Installing new version ${VERSION} of chaincode on peer0.org${ORG}..."
    sleep 15
    set -x
    docker exec -e CORE_PEER_LOCALMSPID=Org${ORG}MSP -e CORE_PEER_ADDRESS=peer0.org${ORG}.example.com:${PEER_PORT} -e CORE_PEER_MSPCONFIGPATH=/opt/gopath/src/github.com/hyperledger/fabric/peer/organizations/peerOrganizations/org${ORG}.example.com/users/Admin@org${ORG}.example.com/msp -e CORE_PEER_TLS_ROOTCERT_FILE=/opt/gopath/src/github.com/hyperledger/fabric/peer/organizations/peerOrganizations/org${ORG}.example.com/peers/peer0.org${ORG}.example.com/tls/ca.crt cli peer lifecycle chaincode install ${CHINCODE_NAME}.tar.gz >&log.txt
    res=$?
    set +x
    cat log.txt
    verifyResult $res "Chaincode installation on peer0.org${ORG} has failed"
    echo "===================== Chaincode version ${VERSION} is installed on peer0.org${ORG} ===================== "
    echo
  else
    echo "Chaincode version ${VERSION} already installed on peer0.org${ORG}"
  fi
}
...
approveForMyOrg() {
  ...
  set -x
  docker exec -e CORE_PEER_LOCALMSPID=Org${ORG}MSP -e CORE_PEER_ADDRESS=peer0.org${ORG}.example.com:${PEER_PORT} -e CORE_PEER_MSPCONFIGPATH=/opt/gopath/src/github.com/hyperledger/fabric/peer/organizations/peerOrganizations/org${ORG}.example.com/users/Admin@org${ORG}.example.com/msp -e CORE_PEER_TLS_ROOTCERT_FILE=/opt/gopath/src/github.com/hyperledger/fabric/peer/organizations/peerOrganizations/org${ORG}.example.com/peers/peer0.org${ORG}.example.com/tls/ca.crt cli peer lifecycle chaincode approveformyorg -o orderer.example.com:7050 --ordererTLSHostnameOverride orderer.example.com --tls true --cafile /opt/gopath/src/github.com/hyperledger/fabric/peer/organizations/ordererOrganizations/example.com/orderers/orderer.example.com/msp/tlscacerts/tlsca.example.com-cert.pem --channelID $CHANNEL_NAME --name ${CHINCODE_NAME} --version ${VERSION} --package-id ${PACKAGE_ID} --sequence 1 >&log.txt
  res=$?
  set +x
  cat log.txt
...
checkCommitReadiness() {
  ...
  # we either get a successful response, or reach MAX RETRY
    ...
    set -x
    docker exec cli peer lifecycle chaincode checkcommitreadiness --channelID $CHANNEL_NAME --name ${CHINCODE_NAME} --version ${VERSION} --sequence 1 --output json >&log.txt
    res=$?
    set +x
    let rc=0
    for var in "$@"
...
commitChaincodeDefinition() {
  ...
  set -x
  docker exec cli peer lifecycle chaincode commit -o orderer.example.com:7050 --ordererTLSHostnameOverride orderer.example.com --tls true --cafile /opt/gopath/src/github.com/hyperledger/fabric/peer/organizations/ordererOrganizations/example.com/orderers/orderer.example.com/msp/tlscacerts/tlsca.example.com-cert.pem --channelID $CHANNEL_NAME --name ${CHINCODE_NAME} --peerAddresses peer0.org1.example.com:7051 --tlsRootCertFiles /opt/gopath/src/github.com/hyperledger/fabric/peer/organizations/peerOrganizations/org1.example.com/peers/peer0.org1.example.com/tls/ca.crt --peerAddresses peer0.org2.example.com:9051 --tlsRootCertFiles /opt/gopath/src/github.com/hyperledger/fabric/peer/organizations/peerOrganizations/org2.example.com/peers/peer0.org2.example.com/tls/ca.crt --peerAddresses peer0.org3.example.com:11051 --tlsRootCertFiles /opt/gopath/src/github.com/hyperledger/fabric/peer/organizations/peerOrganizations/org3.example.com/peers/peer0.org3.example.com/tls/ca.crt --version ${VERSION} --sequence 1 >&log.txt
  res=$?
  set +x
  cat log.txt
...
echo
echo "========= Tea Ledger Network (PLN) contract successfully deployed on channel $CHANNEL_NAME  =========== "
